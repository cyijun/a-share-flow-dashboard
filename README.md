...
按同花顺行业模板跟踪 A 股资金流，同时分析个股绝对净额与市值归一化强度、全市场 RPS、ETF 价格强度和 ETF 申赎资金估算。资金流周期、RPS周期和大市值门槛均可通过命令行修改；每次运行都会保存原始数据、结果、验证报告和自包含网页。

## 能做什么

- 同花顺行业板块与个股净流入、净流出双向榜单。
- 任意正整数交易日资金流窗口，例如 `1,5,10` 或 `2,6,9`。
- 市值不低于指定门槛的个股“累计净额 / 最新总市值”。
...
- 个股：`moneyflow_ths.net_amount` 窗口累计；净流入与净流出分别排序。
- 大市值资金强度：窗口累计净额除以最新交易日 `daily_basic.total_mv`。
- RPS：区间前复权收益在最新交易日全A可比股票中的百分位，最高100；前5%定义为 `RPS >= 95`。
- ETF综合强度：所有已配置RPS窗口的可用RPS均值。
- ETF估算资金流：逐日累计 `(当日份额 - 前一交易日份额) × 当日收盘价`。这是申购赎回估算，不能据此识别央行、汇金等具体交易主体。
- 所有“日”均指交易日，不是自然日。

主要接口文档：[`moneyflow_ind_ths`](https://tushare.pro/document/2?doc_id=320)、[`moneyflow_ths`](https://tushare.pro/document/2?doc_id=343)、[`ths_member`](https://tushare.pro/document/2?doc_id=348)、[`ths_index`](https://tushare.pro/document/2?doc_id=261)。脚本还会使用日线、复权因子、每日指标、基金日线和基金份额等接口。
...
- 市值覆盖、RPS样本、ETF名称与份额覆盖；
- 输出文件是否意外包含 Tushare token。

随后 `validate_market_dashboard.py` 完全绕过生产聚合函数，从 `raw/` 独立复算每一个已配置周期。任何关键复算失败都会使一键命令返回非零状态，阻止 `latest.html` 被更新。
